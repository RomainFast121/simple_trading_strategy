...
def attach_buy_and_hold_metrics(summary, buy_and_hold_summary):
    ...
    summary["B&H_max_drawdown_duration_days"] = buy_and_hold_summary["max_drawdown_duration_days"]
    summary["B&H_max_drawdown_recovery_days"] = buy_and_hold_summary["max_drawdown_recovery_days"]
    summary["B&H_drawdown_10pct_count"] = buy_and_hold_summary["drawdown_10pct_count"]
    summary["B&H_drawdown_10pct_frequency_days"] = buy_and_hold_summary["drawdown_10pct_frequency_days"]
    summary["B&H_drawdown_10pct_avg_duration_days"] = buy_and_hold_summary["drawdown_10pct_avg_duration_days"]
    summary["B&H_drawdown_10pct_avg_recovery_days"] = buy_and_hold_summary["drawdown_10pct_avg_recovery_days"]
...
def make_train_test_walkforward_windows(days, train_days, test_days, step_days, label):
    days = pd.Index(pd.to_datetime(days)).drop_duplicates().sort_values()
    train_days = int(train_days)
    test_days = int(test_days)
    step_days = int(step_days)
    if min(train_days, test_days, step_days) <= 0:
        raise ValueError(f"{label} train_days, test_days, and step_days must be positive integers.")
    if len(days) < 3:
        raise ValueError(f"Not enough dates to create {label} walk-forward splits.")
    if train_days + test_days > len(days):
        raise ValueError(f"{label} walk-forward windows are larger than the available sample.")
    ...
    if not splits:
        raise ValueError(f"No {label} walk-forward splits could be created.")
    return splits
...
def _elapsed_days(start, end):
    if pd.isna(start) or pd.isna(end):
        return np.nan
    return (pd.Timestamp(end) - pd.Timestamp(start)).total_seconds() / (24 * 60 * 60)


def drawdown_summary_metrics(wealth, threshold=0.10):
    ...
    output = {
        "max_drawdown": np.nan,
        "max_drawdown_peak": pd.NaT,
        "max_drawdown_trough": pd.NaT,
        "max_drawdown_recovery": pd.NaT,
        "max_drawdown_recovered": False,
        "max_drawdown_duration_days": np.nan,
        "max_drawdown_recovery_days": np.nan,
        "drawdown_10pct_count": 0,
        "drawdown_10pct_frequency_days": np.nan,
        "drawdown_10pct_avg_duration_days": np.nan,
        "drawdown_10pct_avg_recovery_days": np.nan,
        "drawdown_10pct_unrecovered_count": 0,
    }
    if wealth.empty:
        return output
    ...
    output["max_drawdown"] = drawdown.min()

    trough_time = drawdown.idxmin()
    peak_wealth = running_peak.loc[trough_time]
    prior_wealth = wealth.loc[:trough_time]
    prior_peak_times = prior_wealth.index[prior_wealth.eq(peak_wealth).to_numpy()]
    peak_time = prior_peak_times[-1] if len(prior_peak_times) else wealth.loc[:trough_time].idxmax()
    recovery_candidates = wealth.loc[trough_time:].loc[lambda series: series >= peak_wealth]
    recovery_time = recovery_candidates.index[0] if not recovery_candidates.empty else pd.NaT

    output["max_drawdown_peak"] = peak_time
    output["max_drawdown_trough"] = trough_time
    output["max_drawdown_recovery"] = recovery_time
    output["max_drawdown_recovered"] = pd.notna(recovery_time)
    output["max_drawdown_duration_days"] = _elapsed_days(peak_time, trough_time)
    output["max_drawdown_recovery_days"] = _elapsed_days(trough_time, recovery_time)
    ...
    for position, (timestamp, value) in enumerate(wealth.items()):
        if not in_drawdown:
            if drawdown.loc[timestamp] < 0.0:
                in_drawdown = True
                start_position = position
                peak_value = running_peak.loc[timestamp]
                prior_wealth = wealth.iloc[: position + 1]
                prior_peak_times = prior_wealth.index[prior_wealth.eq(peak_value).to_numpy()]
                peak_time = prior_peak_times[-1] if len(prior_peak_times) else timestamp
            continue

        if value >= peak_value:
            segment = drawdown.iloc[start_position : position + 1]
    ...
    if in_drawdown and start_position is not None:
        segment = drawdown.iloc[start_position:]
        episodes.append(
            {
                "peak": peak_time,
                "trough": segment.idxmin(),
    ...
    threshold_episodes = [episode for episode in episodes if episode["depth"] <= -threshold]
    output["drawdown_10pct_count"] = len(threshold_episodes)
    if threshold_episodes:
        elapsed_days = _elapsed_days(wealth.index[0], wealth.index[-1])
        output["drawdown_10pct_frequency_days"] = elapsed_days / len(threshold_episodes)
        output["drawdown_10pct_avg_duration_days"] = float(
            np.nanmean([
                _elapsed_days(episode["peak"], episode["trough"])
                for episode in threshold_episodes
            ])
        )
        recovery_durations = [
            _elapsed_days(episode["trough"], episode["recovery"])
            for episode in threshold_episodes
            if pd.notna(episode["recovery"])
        ]
        if recovery_durations:
            output["drawdown_10pct_avg_recovery_days"] = float(np.nanmean(recovery_durations))
        output["drawdown_10pct_unrecovered_count"] = sum(
            pd.isna(episode["recovery"]) for episode in threshold_episodes
        )
...
def summarize_returns(init_amount, strategy_returns, fee_cost=None, summary_meta=None, active_mask=None):
    ...
    drawdown_metrics = drawdown_summary_metrics(summary_data["wealth"])
...
def summarize_wealth_curve(init_amount, wealth):
    ...
    return summary_data, {
        "yearly_factor": yearly_factor,
        **drawdown_summary_metrics(summary_data["wealth"]),
    }
...
def calculate_buy_and_hold_baseline(
    ...
    available_assets = close_frame.notna()
    close_frame = close_frame.loc[available_assets.any(axis=1)].copy()
    available_assets = available_assets.reindex(close_frame.index).fillna(False)

    if close_frame.empty:
        raise ValueError("No close data available for buy-and-hold baseline on the requested index.")

    asset_returns = (close_frame / close_frame.shift(1) - 1.0).fillna(0.0)
    asset_log_returns = np.log(close_frame / close_frame.shift(1))
    active_assets = available_assets.sum(axis=1)
    target_weights = available_assets.astype(float).div(active_assets.replace(0, np.nan), axis=0).fillna(0.0)
...
def combine_sleeve_frames(sleeve_frames, init_amount, fees, summary_meta=None):
    ...
    for symbol, merged in merged_frames.items():
        ...
        merged["turnover"] = (
            merged["weighted_position"] - merged["weighted_position_prev"]
        ).abs()
        merged["fee_cost"] = merged["turnover"] * fees
