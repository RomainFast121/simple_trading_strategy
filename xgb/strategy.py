...
class XGBStrategy:
    ...
    def _set_walkforward_params(
        ...
        *,
        ...
        if asset_mode is not None:
            asset_mode = str(asset_mode).lower()
            if asset_mode not in {"cross_asset", "per_asset"}:
                raise ValueError("asset_mode must be 'cross_asset' or 'per_asset'.")
        values = {
    ...
    def _load_predictions(self, require_config_match=False):
        ...
        if diagnostics_path.exists():
            diagnostics = json.loads(diagnostics_path.read_text())
            cached_config = diagnostics.get("walkforward_config") or diagnostics.get("walkforward", {})
            cached_asset_mode = cached_config.get("asset_mode")
            if cached_asset_mode in {"cross_asset", "per_asset"}:
                self.asset_mode = cached_asset_mode
    ...
    def _fit_predict_split(self, train_df, tune_df, test_df, random_state):
    ...
    def _append_metric_row(self, metric_rows, test_df, predictions, split_id, search_result, symbol=None):
    ...
    def _prediction_output_frame(self, source_df, predictions, split_id):
        output = source_df[
            [
                "timestamp",
                "trade_date",
    ...
    def _run_walkforward_per_asset(self, split_model_frame, force=False):
        prediction_frames = []
        metric_rows = []
        ...
        for symbol in self.symbols:
            symbol_frame = split_model_frame.loc[split_model_frame["SYMBOL"] == symbol].copy()
            ...
            try:
                ...
                print(f"Skipping {symbol}: not enough complete rows for per-asset walk-forward.")
                continue

            print(f"Starting {symbol} per-asset XGBoost walk-forward with {len(splits)} split(s).")
            for split_idx, split in enumerate(splits, start=1):
                split_start = time.perf_counter()
                train_df = select_split_frame(symbol_frame, split.train_days)
                tune_df = select_split_frame(symbol_frame, split.tune_days)
                test_df = select_split_frame(symbol_frame, split.test_days)
                if train_df.empty or tune_df.empty or test_df.empty:
                    continue
                result = self._fit_predict_split(train_df, tune_df, test_df, self.random_state)
                key = f"{symbol}:{split.split_id}"
                best_params_by_split[key] = deepcopy(result["search_result"]["best_params"])
                prediction_frames.append(
                    self._prediction_output_frame(test_df, result["predictions"], split.split_id)
                )
                ...
                latest_models[symbol] = result
                total_completed += 1
                print(
                    f"Completed {symbol} split {split_idx}/{len(splits)} in "
                    f"{self._format_seconds(time.perf_counter() - split_start)}."
                )

        live_frames = []
        for symbol, result in latest_models.items():
            live_frame = self.feature_panel.loc[
                (self.feature_panel["SYMBOL"] == symbol)
                & self.feature_panel["target_next_log_return"].isna()
            ].copy()
            live_frame = live_frame.dropna(subset=self.feature_columns)
            if live_frame.empty:
                continue
            live_scaled = transform_with_scaler(live_frame, result["scaler"], self.feature_columns)
            if self.pca_enabled and result["pca"] is not None:
                live_scaled = transform_with_pca(live_scaled, result["pca"], result["modeling_columns"])
            ...
            live_output["prediction"] = result["model"].predict(live_scaled[result["modeling_columns"]])
            live_output["split_id"] = "live"
            live_frames.append(live_output)

        if live_frames:
            prediction_frames.extend(live_frames)
        if not prediction_frames:
            raise ValueError("No per-asset XGB predictions were generated.")
        ...
        print(
            f"Finished per-asset XGBoost walk-forward with {total_completed} fitted split(s) "
            f"in {self._format_seconds(time.perf_counter() - run_start)}."
        )
    ...
    def run_walkforward(
        ...
        *,
        ...
        if self.asset_mode == "per_asset":
            if not force and self.predictions.empty and self._load_predictions(require_config_match=True):
                print(f"Loaded cached per-asset XGB predictions from {self.output_dir}.")
                return self.predictions
            return self._run_walkforward_per_asset(split_model_frame, force=force)
        ...
        for split_idx, split in enumerate(splits, start=1):
            ...
            latest_model, latest_scaler, latest_pca = result["model"], result["scaler"], result["pca"]
            latest_modeling_columns = result["modeling_columns"]

            prediction_frames.append(self._prediction_output_frame(test_df, predictions, split.split_id))
            self._append_metric_row(metric_rows, test_df, predictions, split.split_id, search_result)
    ...
    def _score_candidate_fast(self, prediction_frame, threshold_quantile, vol_lookback, threshold_value=None):
        ...
        turnover = weighted_positions.diff().abs()
        if not turnover.empty:
            turnover.iloc[0] = weighted_positions.iloc[0].abs()
        weighted_turnover = turnover.fillna(0.0).sum(axis=1)
        ...
        return {
            ...
            **drawdown_summary_metrics(wealth),
        }

    def _build_parameter_schedule_for_frame(self, prediction_frame, symbol=None):
        ...
        for split_position, split_id in enumerate(split_ids):
            ...
            if split_position == 0:
                ...
                if not self._suppress_schedule_prints:
                    print(
                        f"{symbol + ' ' if symbol else ''}Split {split_id}: "
                        f"default threshold quantile={self.threshold_quantile:.3f}, "
            ...
            if not self._suppress_schedule_prints:
                print(
                    f"{symbol + ' ' if symbol else ''}Split {split_id}: "
                    f"selected threshold quantile={float(best['threshold_quantile']):.3f}, "
                    f"lookback={int(best['vol_lookback'])} from prior splits."
                )
        return pd.DataFrame(schedule_rows), pd.DataFrame(sweep_rows)

    def _build_parameter_schedule(self, prediction_frame):
        if self.asset_mode == "per_asset":
            schedule_parts = []
            sweep_parts = []
            for symbol, symbol_frame in prediction_frame.loc[
                prediction_frame["split_id"].astype(str) != "live"
            ].groupby("SYMBOL", sort=False):
                schedule, sweep = self._build_parameter_schedule_for_frame(symbol_frame.copy(), symbol=symbol)
            ...
            self.parameter_schedule = (
                ...
                .sort_values(["SYMBOL", "split_id"])
                .reset_index(drop=True)
                if schedule_parts
                else pd.DataFrame()
            )
            self.parameter_sweep = (
                pd.concat(sweep_parts, ignore_index=True).reset_index(drop=True)
                if sweep_parts
                else pd.DataFrame()
            )
            return self.parameter_schedule

        self.parameter_schedule, self.parameter_sweep = self._build_parameter_schedule_for_frame(prediction_frame)
        self.parameter_schedule = self.parameter_schedule.sort_values("split_id").reset_index(drop=True)
        self.parameter_sweep = self.parameter_sweep.reset_index(drop=True)
        return self.parameter_schedule
    ...
    def _build_split_performance(self, prediction_frame):
        ...
        for _, schedule_row in self.parameter_schedule.iterrows():
            ...
            mask = prediction_frame["split_id"] == split_id
            if self.asset_mode == "per_asset" and "SYMBOL" in schedule_row:
                mask &= prediction_frame["SYMBOL"] == schedule_row["SYMBOL"]
            split_frame = prediction_frame.loc[mask].copy()
            ...
            rows.append(
                {
                    ...
                    **({"SYMBOL": schedule_row["SYMBOL"]} if self.asset_mode == "per_asset" and "SYMBOL" in schedule_row else {}),
                    **row,
                }
            )
    ...
    def _evaluate_predictions(self):
        ...
        merge_keys = ["split_id"]
        if self.asset_mode == "per_asset":
            schedule_columns.insert(1, "SYMBOL")
            merge_keys = ["split_id", "SYMBOL"]
        prediction_frame = prediction_frame.merge(
            self.parameter_schedule[schedule_columns],
            on=merge_keys,
            how="left",
        )
        if "live" in prediction_frame["split_id"].astype(str).values and not self.parameter_schedule.empty:
            ...
            if self.asset_mode == "per_asset":
                for symbol, symbol_schedule in self.parameter_schedule.groupby("SYMBOL", sort=False):
                    if symbol_schedule.empty:
                        continue
                    last = symbol_schedule.iloc[-1]
                    symbol_live = live & (prediction_frame["SYMBOL"] == symbol)
                    prediction_frame.loc[symbol_live, "applied_threshold_quantile"] = float(
                        last["applied_threshold_quantile"]
                    )
                    prediction_frame.loc[symbol_live, "applied_threshold_value"] = float(
                        last["applied_threshold_value"]
                    )
                    prediction_frame.loc[symbol_live, "threshold_reference"] = str(last["threshold_reference"])
                    prediction_frame.loc[symbol_live, "applied_vol_lookback"] = int(last["applied_vol_lookback"])
            else:
        ...
        group_keys = ["SYMBOL", "split_id"] if self.asset_mode == "per_asset" else ["split_id"]
        for _, split_frame in prediction_frame.groupby(group_keys, sort=False):
    ...
    def latest_positions(self):
        ...
        if self.asset_mode == "per_asset":
            latest = latest.merge(
                self.parameter_schedule.sort_values(["SYMBOL", "split_id"])
            ...
            latest["applied_threshold_quantile"] = float(last_schedule["applied_threshold_quantile"])
            latest["applied_threshold_value"] = float(last_schedule["applied_threshold_value"])
            latest["threshold_reference"] = str(last_schedule.get("threshold_reference", "parameter_schedule"))
            latest["applied_vol_lookback"] = int(last_schedule["applied_vol_lookback"])

        lookbacks = sorted(
            pd.to_numeric(latest["applied_vol_lookback"], errors="coerce")
            .dropna()
            .astype(int)
            .unique()
        )
        volatility_panel = estimate_asset_annualized_volatility(
            ...
            lookback_bars=lookbacks[0] if lookbacks else self.vol_lookback,
            return_col="simple_return",
        ...
        latest = latest.merge(
            volatility_panel.drop(columns=["simple_return"], errors="ignore"),
            on=["timestamp", "SYMBOL"],
            how="left",
        )
        if len(lookbacks) > 1:
            latest["asset_vol_annualized"] = latest.apply(
                lambda row: row.get(
                    "asset_vol_annualized"
                    if int(row["applied_vol_lookback"]) == lookbacks[0]
                    else f"asset_vol_annualized_{int(row['applied_vol_lookback'])}"
                ),
                axis=1,
            )
        predictions = pd.to_numeric(latest["prediction"], errors="coerce")
        latest["abs_prediction"] = predictions.abs()
        latest["is_active_signal"] = (
            predictions.abs().ge(latest["applied_threshold_value"].astype(float))
            & predictions.ne(0.0)
        )
        ...
        latest["threshold_quantile"] = latest["applied_threshold_quantile"]
        latest["threshold_value"] = latest["applied_threshold_value"]
        latest["threshold_source"] = latest["threshold_reference"]
        latest["vol_lookback"] = latest["applied_vol_lookback"]
        return latest[
            [
                "available_at",
                "applies_to",
