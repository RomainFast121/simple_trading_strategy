...
class MomentumStrategy:
    ...
    def _evaluate_multi_ticker(self, close_map):
        ...
        evaluation_start = max(frame.index[0] for frame in sleeve_frames.values())
        sleeve_frames = {
            ticker_name: frame.loc[frame.index >= evaluation_start].copy()
            for ticker_name, frame in sleeve_frames.items()
        }
